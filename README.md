...
| 字段 | 说明 | 示例 |
| :--- | :--- | :--- |
| **pansou 服务地址** | 你部署好的地址（结尾不要带 `/`）。**支持逗号分隔多个地址** | `http://pansou-app:80,https://pansou.yourdomain.workers.dev` |
| **访问鉴权 Token** | 若 Worker 里配了 `AUTH_TOKEN` 则填写 | `your_secret_token` |
| **启用网盘聚合搜索** | 勾选后生效 | ✅ |

填写后点击 **「🔌 测试连接」**，系统会**逐个地址**探测并给出「耗时 + 实际检索到的条数」——
只连通但搜不到数据的地址也会被明确标出来。
...
```yaml
environment:
  - PANSOU_EDGE_URL=http://pansou-app:80,https://pansou.yourdomain.workers.dev
  - PANSOU_EDGE_TOKEN=your_secret_token
```

#### ⚠️ 多地址容错（v1.2 起）

配置**多个地址**（逗号 / 分号分隔）时，它们会**并行竞速、互为兜底**：
先返回非空结果者胜出，其余自动取消。任一地址可用就不会出现「网盘一条都搜不到」。

> **踩坑提醒**：网页「系统设置」里保存的值**优先级高于 docker-compose 的环境变量**。
> 如果你在网页里把地址改成了某个不可达的远端地址（例如远程 Worker 需要 30 秒才响应），
> 会**覆盖掉容器里正确的本机配置**，导致网盘搜索整体失效。
> 遇到「网盘搜不到」时，第一件事就是打开「⚙️ 系统设置」点一下「测试连接」，
> 看看每个地址的耗时和条数是否正常；必要时点「恢复默认」清掉网页覆盖值。

#### 网盘线路的呈现方式

搜索「繁花」时，网盘结果会**并入同名影视主卡片**，成为该影片的额外线路：

```
【卡片】繁花   备注：已完结 · 含173个网盘资源
   ├─ 非凡高清        （切片站秒播，30 个资源）
   ├─ 夸克4K原盘      （网盘，15 个资源）
   ├─ 阿里高清网盘     （网盘，15 个资源）
   ├─ 百度网盘        （网盘，15 个资源）
...
若某部片只有网盘有、切片站没有，则会保留为独立的 `【XX网盘】片名` 卡片。
